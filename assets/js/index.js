...
document.querySelectorAll('.navigations a').forEach(link => {
...
const txtOuterElements = document.querySelectorAll('.porTxtptitleOuter');
const workNumElements = document.querySelectorAll('.worknum');
...
txtOuterElements.forEach(el => observer.observe(el));
workNumElements.forEach(el => observer.observe(el));



// Select all header elements
const headerElements = document.querySelectorAll('.porHeadcon h1, .porHeadcon p');
...
const headerObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if(entry.isIntersecting){
      entry.target.classList.add('animate');
      headerObserver.unobserve(entry.target); // animate only once
    }
  });
}, { threshold: 0.2 });
